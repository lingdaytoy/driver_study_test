 ...
 *  处理并发与竞争的方法
 *      信号量
 *          通过信号量值来控制对共享资源的访问数量，如果需要共享资源，需要先获取信号量值
 *          若信号量值满，信号量会使等待资源进入休眠状态，直到有空余的信号量值
 * @Author: dwl
 * @Date: 2024-12-07 09:41:53
 * @LastEditTime: 2024-12-07 11:44:59
 * @LastEditors: Do not edit
 * @FilePath: /character_device/12_07_semaphore/semaphore.c
 */
...
static int gpioled_open(struct inode *inode, struct file *file)
{
    ...
    // 获取信号量值，进入休眠状态的进程可以被信号打断
    if (down_interruptible(&gpioled.semaphore))
    {
        return -ERESTARTSYS;
    }

#if 0
    down(&gpioled.semaphore); // 不能被信号打断
#endif
...
static int __init gpioled_init(void)
{
    ...
    sema_init(&gpioled.semaphore, 1); // 初始化信号量值
